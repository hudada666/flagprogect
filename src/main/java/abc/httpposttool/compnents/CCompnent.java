...
@Component
public class CCompnent {
    @Autowired
    CommomUtil commomUtil;
    @Autowired
    ComponentDetails componentDetails;

    Logger logger = LoggerFactory.getLogger("CCompnent");
    ...
    public Map<String,Object> compFlow(Map<String,Object> dataContant) {
        ...
        String flagCode = appenMap.get("C");
        String compCode = "C";
        String compRespose;
        if (appenMap.get(compCode) != null && !"".equals(appenMap.get(compCode))){
            Date data = new Date();
            if (flagCode.equals("1")) {
                String msg = "桩点:交易码：" +  dataContant.get("strnCode") + " 组件：CCompnent" + " 装点信息：" + flagCode;
                logger.info("桩点:交易码：" +  dataContant.get("strnCode") + " 组件：CCompnent" + " 装点信息：" + flagCode);
