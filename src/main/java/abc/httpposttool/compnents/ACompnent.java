...
public class ACompnent {
    ...
    Logger logger = LoggerFactory.getLogger("ACompnent");
    ...
    public Map<String,Object> compFlow(Map<String,Object> dataContant) {
        ...
        String flagCode = appenMap.get("A");
        String compCode = "A";
        String compRespose;
        if (appenMap.get(compCode) != null && !"".equals(appenMap.get(compCode))){
            Date data = new Date();
            if (flagCode.equals("1")) {
                String msg = "桩点:交易码：" +  dataContant.get("strnCode") + " 组件：ACompnent" + " 装点信息：" + flagCode;
                logger.info("桩点:交易码：" +  dataContant.get("strnCode") + " 组件：ACompnent" + " 装点信息：" + flagCode);
